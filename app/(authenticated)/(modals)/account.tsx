...
const Page = () => {
  ...
  const onSaveUser = async () => {
    try {
      await clerkUser?.update({ firstName: firstName!, lastName: lastName! });
      setEdit(false);
      updateUser({ firstName, lastName }); // Actualizar el contexto
    } catch (error) {
      console.error(error);
    } finally {
      setEdit(false);
  ...
  const onCaptureImage = async () => {
    ...
    if (!result.canceled) {
      ...
      updateUser({ imageUrl: base64 }); // Actualizar el contexto
    }
  };
